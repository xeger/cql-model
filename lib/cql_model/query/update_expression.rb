  ...
  class UpdateExpression < Expression
    ...
    def __build__
      if @left.nil? || @operator.nil? || @right.nil?
        ...
        case @operator
        when :[]=
          key = ::CQLModel::Query.cql_value(@right[0], context=:update)
          val = ::CQLModel::Query.cql_value(@right[1], context=:update)
          "#{left}[#{key}] = #{val}"
        else
          op    = OPERATORS[@operator]
          right = ::CQLModel::Query.cql_value(@right, context=:update)
          "#{left} #{op} #{right}"
        end
      end
    end
